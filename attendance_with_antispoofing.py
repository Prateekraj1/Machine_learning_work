...
def login():
                ...
                def manage_employee():
                    try:
                        ...
                        def add_employee():
                            ...
                            if post_var.get() == "" or fname_var.get() == "" or gender_var.get() ==  "" or contact_var.get() == "" or address_var.get() == "" or Reg_no.get()=="":
                                ...
                                if (re.search('[a-zA-Z]+', fname_var.get())):
                                    if len(contact_var.get()) != 10:
                                        ...
                                        if (re.search('[0-9]$', contact_var.get())):
                                            regex = '^(\w|\.|\_|\-)+[@](\w|\_|\-|\.)+[.]\w{2,3}$'
                                            if(re.search(regex, address_var.get())):
                                                ...
                                                if not os.path.exists(input_directory):
                                                    ...
                                                    cur1 = conn.cursor()
                                                    cur1.execute("insert into attendance(department,fname,gender,contact_no,email,reg_no) VALUES (%s,%s,%s,%s,%s,%s)", (
                                                                                                                                                                                post_var.get(),
                                                                                                                                                                                fname_var.get(),
                                                                                                                                                                                gender_var.get(),
                                                                                                                                                                                contact_var.get(),
                                                    ...
                                                    conn.commit()
                                                    cur2 = conn.cursor()
                                                    cur2.execute("select sid from attendance where fname=%s ",(name))
                                            ...
                                            if(re.search(regex, address_var.get())):
                                                ...
                                                if id in staff_ids:
                                                    ...
                                                    if (q == True):
                                                        ...
                                                        video_capture.release()
                                                        cv2.destroyAllWindows()
                                                        cur.execute("update attendance set department = %s, fname = %s, gender = %s, contact_no = %s, email_address = %s where sid = %s", (                                                               
                                                                                                    post_var.get(),
                                                                                                    fname_var.get(),
                                                                                                    gender_var.get(),
                                                                                                    contact_var.get(),
                    ...
                    ###################### To delete the items #########################
                        def delete():
                            ...
                            if post_var.get() == "" or fname_var.get() == "" or gender_var.get() ==  "" or contact_var.get() == "" or address_var.get() == "" or Reg_no.get()=="":
                                ...
                                try:
                                    input_name = fname_var.get() + "_" + sid_var.get()
                                    staff_input = os.path.join(dataset_dir,input_name)
                                    if not os.path.exists(staff_input):
                                        cur.execute("delete from attendance where sid = %s",sid_var.get())
                                    else:
                                        cur.execute("delete from attendance where sid = %s",sid_var.get())
                    ...
                    ################################################## Student Management form ###############################
                        ...
                        E1 = Entry(f2,state="disabled", width = 20, textvariable = sid_var,  font = ("italic",13, "bold") ).place(x = 180  , y = 100)
                        post = Label(f2, text = "Department", bg = "gray",  font = ("italic",13, "bold")).place(x = 35, y = 150 )
                        E2 = Entry(f2, width = 20, textvariable = post_var,  font = ("italic",13, "bold")).place(x =180, y = 150)
                        fname = Label(f2, text = "Full Name", bg = "gray", font = ("italic",13, "bold")).place(x =35, y = 200)
                        ...
                        reg_no = Label(f2, text = "Registeration Number", bg = "gray",font = ("italic",12, "bold")).place(x = 35, y = 400 )
                        E6 = Entry(f2, textvariable = Reg_no , font = ("italic",12, "bold")).place(x = 180, y = 400)
                        f2.place(x = 10, y = 90)
                    ...
                    ################################################################################### Large Frame
                        ...
                        c1 = Combobox(first, textvariable = search_from, values = ["sid","fname","department"], state = "readonly", width = "25").place(x = 580, y = 109)
                    ...
                    ################################################################################## Table frame
                        ...
                        table1 = Treeview(f5, columns = ("sid","post", "fname","gender","contact.no","address","reg_no"), xscrollcommand = scroll_x.set, yscrollcommand = scroll_y.set)
                        scroll_x.pack(side = BOTTOM, fill = X )
                        scroll_y.pack(side = RIGHT, fill = Y)
                        scroll_x.config(command = table1.xview)
                        scroll_y.config(command = table1.yview)
                        table1.heading("sid", text ="student_id")
                        ...
                        table1.heading("reg_no", text= "Reg_no")
                        table1['show'] = 'headings'
                        table1.column("sid", width = 119)
                        ...
                        table1.column("reg_no", width = 119)
    ...
    ######################################### Function to recognize the face
                ...
                attendance = Tk()
                attendance.title("Biometric Machine")
                ...
                manage = 'Smart Biometric Machine By ArtifcialX'
...
title = Label(face, text = "Login Page" , font = ("times new roman", 30, "bold"), bg = "green", fg = "yellow", bd = 7, relief = GROOVE) 
